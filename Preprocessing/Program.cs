    ...
    class Program
    {
        ...
        private static async Task Predict(string fileName)
        {
            using (Image<Rgba32> image = Image.Load(fileName))
            {
                ...
                await PredictDigitWithMLStudioAsync(_client, image);
            }
        }
        ...
        private static int[] ConvertImageToArray(Image<Rgba32> image)
        {
        ...
        private static async Task<int> PredictDigitWithMLStudioAsync(HttpClient client, Image<Rgba32> i)
        {
            ...
            var prediction = JsonConvert.DeserializeObject<Rootobject>(responseContent);
